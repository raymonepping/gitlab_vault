...
Options:
  ...
  --retrieve-audit-file <path>         Copy the audit file from the Vault container and use it as input
  --redact                             Redact sensitive details in output. Default mode: pseudo
  --redact-mode <pseudo|mask|strict>   Redaction mode. Default: pseudo
...
Examples:
  ...
  ./parse_vault.sh ./vault_audit.log --secrets-only --redact --format json
  ./parse_vault.sh ./vault_audit.log --secrets-only --redact --redact-mode strict --format md
EOF
}
  ...
  if type == "object" then
    with_entries(
      ...
      | if (.key | IN("user_login","user_email","user_id","display_name","project_path","namespace_path","pipeline_id","job_id","ref","path","entity_id","source_file","retrieved_audit_file","role")) then
          .value = "[redacted]"
        elif (.key | IN("display_names","entity_ids","projects","namespaces","pipelines","jobs","refs","paths")) then
          .value = ["[redacted]"]
        else .
        end
    )
